...
of the one automatically generated by cqfd.
...
distant source. For example, to use Ubuntu 16.04, it can be set like:
...
Unless `tag` is used, Docker images generated by cqfd will be named
`cqfd-%Un-%Po-%Pn-%Dh` (`cqfd-%Un-%Po-%Pn-%Dh-%Bd` if `distro` is set).

Note: When using a prebuilt image from a repository, the cqfd init command
and dockerfile are **not** necessary as no image has to be built.
