...
``tag`` (optional): allows specifying a custom Docker image name to use instead of the one automatically generated by cqfd.

``image`` (optional): allows specifying a Docker image to pull from a distant source. Ex: Ubuntu:20:04

Note: **When using a prebuilt image from a repository, the cqfd init command and dockerfile are not necessary** as no image has to be built.

### Docker image naming ###

Unless ``tag`` is used, Docker images generated by cqfd will be
